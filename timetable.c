...
void initCECSV() {
    ...
    if (fp == NULL) {
        ...
        fprintf(fp, "Course,Day,Time,Hall,Lecturer\n");
        fclose(fp);
    } else {
        fclose(fp);
    }
...
void saveCEToCSV(
	...
	void generateTimeTable() {
		...
		printf("+----------------------+----------+------------+----------+----------------------+\n");
	      ...
	      for (i = 0; i < count; i++) {
	        printf("| %-20s | %-8s | %-10s | %-8s | %-20s |\n",
	               course[i], day[i], time[i], hall[i], lecturer[i]);
	    printf("+----------------------+----------+------------+----------+----------------------+\n");
       ...
       // THIS IS WHERE THE CSV FILE TRANSPORTATION AND EXPORTATIONS BEGINS
			...
			void saveEEToCSV(
			...
			)
			 {
			    ...
			    fprintf(fp, "%s,%s,%s,%s,%s\n",
			            ee_course, ee_day, ee_time, ee_hall, ee_lecturer);
 	     ...
 	     if(ee_count == 0){
 	     	...
 	     	return;
	              ...
	              printf("+----------------------+----------+------------+----------+----------------------+\n");
 	              ...
 	              for(i = 0; i < ee_count; i++){
 	              	printf("| %-20s | %-8s | %-10s | %-8s | %-20s |\n",
					       ee_Course[i], ee_Day[i], ee_Time[i], ee_Hall[i], ee_Lecturer[i]);
			...
			void initMECSV() {
					...
					void saveMEToCSV(
					...
					)
					 {
					    ...
					    fprintf(fp, "%s,%s,%s,%s,%s\n",
					            me_course, me_day, me_time, me_hall, me_lecturer);
 	     ...
 	     if(me_count == 0){
 	     	...
 	     	return;
	              ...
	              printf("+----------------------+----------+------------+----------+----------------------+\n");
 	              ...
 	              for(i = 0; i < me_count; i++){
 	              	printf("| %-20s | %-8s | %-10s | %-8s | %-20s|\n",
					       me_course[i], me_day[i], me_time[i], me_hall[i], me_lecturer[i]);
	...
	int civ_count = 0;
			...
			void saveCIVToCSV(
			...
			)
			 {
			    ...
			    fprintf(fp, "%s,%s,%s,%s,%s\n",
			            civ_course, civ_day, civ_time, civ_hall, civ_lecturer);
			...
			void initCPECSV() {
					...
					void saveCPEToCSV(
					...
					)
					 {
					    ...
					    fprintf(fp, "%s,%s,%s,%s,%s\n",
					            cpe_course, cpe_day, cpe_time, cpe_hall, cpe_lecturer);
 	     ...
 	     if(cpe_count == 0){
 	     	...
 	     	return;
	              ...
	              printf("+----------------------+----------+------------+----------+----------------------+\n");
 	              ...
 	              for(i = 0; i < cpe_count; i++){
 	              	printf("| %-20s | %-8s | %-10s | %-8s | %-20s |\n",
					       cpe_course[i], cpe_day[i], cpe_time[i], cpe_hall[i], cpe_lecturer[i]);
